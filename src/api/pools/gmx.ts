...
export const zapIn: ZapInFn = async ({ amountInWei, balances, token, currentWallet, signer, chainId, max }) => {
    if (!signer) return;
    ...
    console.log({ BLOCK_EXPLORER_URL, blockExplorersByChainId, chainId });
    ...
    try {
        ...
        if (token === constants.AddressZero) {
            ...
            console.log({ vault_addr: farm.vault_addr, token, amountInWei });
...
export const zapOut: ZapOutFn = async ({ amountInWei, token, currentWallet, signer, chainId, max }) => {
    if (!signer) return;
    const zapperContract = new Contract(farm.zapper_addr, farm.zapper_abi, signer);
    ...
    try {
        ...
        if (token === constants.AddressZero) {
            withdrawTxn = await zapperContract.zapOutAndSwapEth(farm.vault_addr, max ? vaultBalance : amountInWei, 0);
        } else {
