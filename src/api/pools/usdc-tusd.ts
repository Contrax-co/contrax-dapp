...
const farm = pools.find((farm) => farm.id === 12) as Farm;
...
export const getFarmData = async (
    ...
    const usdcPrice = await getPrice(farm.token1, defaultChainId);
    const usdcBalance = await getBalance(farm.token1, currentWallet, provider);
    ...
    farmData = {
        ...
        Max_Zap_Withdraw_Balance: ((Number(toEth(vaultBalance)) * lpPrice) / usdcPrice).toString(),
        ...
        Max_Zap_Deposit_Balance_Dollar: (Number(toEth(usdcBalance, farm.decimals1)) * usdcPrice).toString(),
        Max_Zap_Deposit_Balance: toEth(usdcBalance, farm.decimals1),
        Token_Token_Symbol: farm.name,
        Zap_Token_Symbol: "USDC",
