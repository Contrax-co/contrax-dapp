    ...
    if (currentWallet) {
        ...
        try {
            if (ethereum) {
                ...
                let exchangeTxn;
                try {
                    const gasEstimated: any = await exchangeContract.estimateGas.swapFromTokenToToken(
                        from,
                        to,
                        formattedBal
                    );
    ...
    if (currentWallet) {
        ...
        try {
            if (ethereum) {
                ...
                let exchangeTxn;
                try {
                    const gasEstimated: any = await exchangeContract.estimateGas.swapTokenForPair(
                        from,
                        to,
                        formattedBal
                    );
    ...
    if (currentWallet) {
        ...
        try {
            if (ethereum) {
                ...
                let exchangeTxn;
                try {
                    const gasEstimated: any = await exchangeContract.estimateGas.swapPairForToken(
                        from,
                        to,
                        formattedBal
                    );
    ...
    if (currentWallet) {
        ...
        try {
            if (ethereum) {
                ...
                try {
                    const gasEstimated: any = await exchangeContract.estimateGas.swapPairForPair(
                        from,
                        to,
                        formattedBal
                    );
    ...
    if (currentWallet) {
        ...
        try {
            if (ethereum) {
                ...
                let exchangeTxn;
                try {
                    const gasEstimated: any = await exchangeContract.estimateGas.swapFromEthToToken(to, {
                        value: formattedBal,
                    });
                    const gasMargin = gasEstimated * 1.1;
    ...
    if (currentWallet) {
        ...
        try {
            if (ethereum) {
                ...
                let exchangeTxn;
                try {
                    const gasEstimated: any = await exchangeContract.estimateGas.swapEthForPair(to, {
                        value: formattedBal,
                    });
                    const gasMargin = gasEstimated * 1.1;
        ...
        try {
            if (ethereum) {
                ...
                try {
                    ...
                    exchangeTxn = await exchangeContract.swapPairForEth(from, formattedBal, {
                        gasLimit: Math.ceil(gasMargin),
                    });
                } catch {
                    exchangeTxn = await exchangeContract.swapPairForEth(from, formattedBal, {
