...
const FarmDetailsLp = ({ farm }: { farm: Farm }) => {
    const downsampleData = (data: LP_Prices[]) => {
        ...
        for (const key in tempMap) {
            ...
            monthlyData.push({ date: key, lp: averageApy });
        }
    ...
    const newData = downsampleData(lp);
    const monthNames = [
        "January",
        "February",
        "March",
        ...
        <div className={"apyContainer"}>
            ...
            <div className={"specificApy"}>
                ...
                {isLpPriceLoading ? (
                    ...
                    <p className={`apy--light ${lightMode && "apy--dark"}`}>{averageLp.toFixed(2)}%</p>
                )}
            </div>
            <div style={{ marginTop: "10px", width: "100%", height: "250px" }}>
                <p className={`apy--light ${lightMode && "apy--dark"}`} style={{ marginBottom: "10px" }}>
