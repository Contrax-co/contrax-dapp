...
export const estimateValueTo = async(
  fromAmt:any,
  fromPrice: any, toPrice:any,
  tokenType1:any, tokenType2: any,
  tokenName:any, toName:any,
  value:any, fromAddress: any, 
  toAddress:any, setToValue:any
) => {
  const { ethereum } = window;
  try{
    if(ethereum){

      if(tokenName === "ETH" && toName === "WETH"){
        setToValue(value);
      }
      else if (tokenName === "WETH" && toName === "ETH"){
        setToValue(value);
      }
      else if(tokenType1 === "Token" && tokenType2 === "LP Token"){
        setToValue((fromPrice * fromAmt)/toPrice);
      }
      else if(tokenType1 === "Token" && (toName === "WETH" || toName === "ETH")){
        const provider = new ethers.providers.Web3Provider(ethereum);
        await provider.send('eth_requestAccounts', []);
        const signer = provider.getSigner();
      ...
      else if (tokenType1 === "Token" && tokenType2 === "Token" && (toName !== "WETH" || toName !== "ETH")) {
        const provider = new ethers.providers.Web3Provider(ethereum);
        await provider.send('eth_requestAccounts', []);
        const signer = provider.getSigner();
        const Uniswap = new ethers.Contract(uniSwap_address, uniswap_abi, signer); 
