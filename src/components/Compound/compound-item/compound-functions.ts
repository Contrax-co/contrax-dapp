...
export const totalLPTokenExisting = async(pool:any, price0: any, price1: any, setPriceOfToken:any) => {
    const {ethereum} = window; 
    try{
        if(ethereum){
            const provider = new ethers.providers.Web3Provider(ethereum);
...
export const usdTokenValueInVault = async(priceOfToken: any, totalVaultBalance: any, setValueInVault: any) => {
    const valueInVault = priceOfToken * totalVaultBalance; 
    setValueInVault(valueInVault); 
}

export const usdUserVaultValue = async(priceOfToken: any, userVaultBalance:any, setUserUsdValue:any) => {
