...
## Decide Which Farms you want to join

Go to the farms page and see the different farms offered. When deciding what farms to join, don't just look at the APY. Consider the underlying assets and how the values can change. Ater all, your positions are subject to volatility of the one or two tokens that make up your position.

We do offer two stablecoin farms that are not subject to market changes as they represent USD, while many others are half ETH and half stablecois, so only one side is subject to market changes. When an asset changes value, your staking is subject to impermanent loss, which you can learn more about in [this article](https://medium.com/coinmonks/understanding-impermanent-loss-9ac6795e5baa).

There are two types of farms: single-sided staking (GMX, USDC, and USDT) and LP staking (the rest). The single sided requires you to swap into the token, but for the LP farms, you can skip straight to depositing.

## Swapping your ETH for a singe sided token
...
4. Enter the amount of ETH that you want to swap.
