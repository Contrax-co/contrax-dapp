...
The LP token is a receipt to show you provided liquidity, but to earn rewards on it, you need to stake it in a farm. Contrax uses a technology called Zapping which allows you to deposit your USDC or ETH and have it auto-convert into the farm's LP token tokens. You will not have to worry about dealing with any token except your USDC or ETH.

## Yield and APY

Yield refers to the estimated rewards you receive from providing liquidity. In a farm, you are earning rewards from trading fees and from the platform rewarding you for using their pool. Contrax uses the numbers the platform provides to calculate your _APY_, or annual percentage yield. So a $1000 deposited in a farm with 20% APY should be around $1,200 by the end of the year if the APY and token price don't change.
